...
import (
	...
	nsqConsumer "github.com/MISingularity/deepshare2/deepstats/console/nsq_consumer/appevent"
...
func main() {
	...
	nsqChannel := fs.String("channel", "deepstats_appevent", "Specify the NSQ channel for consumer")
	mongoDB := fs.String("mongodb", "deepstats", "Specify the Mongo database")
	mongoColl := fs.String("mongocoll", "appevent", "Specify the Mongo collection")

	mongoAddr := fs.String("mongo-addr", "", "Specify the raw data mongo database URL")
	log.InitLog("[DEEPSTATS][APPEVENT]", "", log.LevelDebug)
