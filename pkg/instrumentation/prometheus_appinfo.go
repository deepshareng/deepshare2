...
// PrometheusAppInfo holds metrics for all appinfo methods.
...
func (ps *prometheusAppInfo) HTTPGetDuration(start time.Time) {
	ps.httpGetDuration.Observe(float64(time.Since(start)) / float64(time.Millisecond))
}
func (ps *prometheusAppInfo) HTTPPutDuration(start time.Time) {
	ps.httpPutDuration.Observe(float64(time.Since(start)) / float64(time.Millisecond))
}

func (ps *prometheusAppInfo) StorageGetDuration(start time.Time) {
	ps.storageGetDuration.Observe(float64(time.Since(start)) / float64(time.Millisecond))
}

func (ps *prometheusAppInfo) StoragePutDuration(start time.Time) {
